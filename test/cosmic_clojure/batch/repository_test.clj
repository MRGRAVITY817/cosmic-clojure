...
(deftest repository-test
  (testing "can save and retrieve a batch"
    (let [;; Arrange
          repo    (xtdb/batch-repo (xtdb-client {:type :in-process}))
          eta     (now)
  ...
  (testing "can save and retreive a batch with allocations"
    (let [;; Arrange
          ...
          batch   (batches/allocate-line
                   (batches/->batch {:reference "batch-002",
                                     :sku       "SMALL-TABLE",
                                     :quantity  10,
                                     :eta       eta})
      ...
      ;; Assert
      (is (= output
             [{:Batch/reference   "batch-002",
               :Batch/sku         "SMALL-TABLE",
               :Batch/purchased-quantity 10,
               :Batch/eta         eta,
               :Batch/allocations #{(batches/->order-line {:order-id "order-002",
                                                           :sku      "SMALL-TABLE",
                                                           :quantity 1})}}]))))
  (testing "save and get batch by reference"
    (let [;; Arrange
          ...
          eta     (now)
          batch   (batches/allocate-line
                   (batches/->batch {:reference "batch-003",
                                     :sku       "SMALL-TABLE",
                                     :quantity  10,
                                     :eta       eta})
      ...
      ;; Assert
      (is (= output
             {:Batch/reference   "batch-003",
              :Batch/sku         "SMALL-TABLE",
              :Batch/purchased-quantity 10,
              :Batch/eta         eta,
              :Batch/allocations #{(batches/->order-line {:order-id "order-003",
                                                          :sku      "SMALL-TABLE",
                                                          :quantity 1})}})))))
